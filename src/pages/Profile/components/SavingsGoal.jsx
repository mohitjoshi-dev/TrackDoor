...
export default function SavingsGoal() {
  ...
  useEffect(() => {
    if (profile?.savings_goal !== null && profile?.savings_goal !== undefined) {
  ...
  ? Math.max(savingsGoal - displaySavings, 0)
  : 0;
  
  const progress = savingsGoal
  ? Math.min((displaySavings / savingsGoal) * 100, 100)
  : 0;
          ...
          <div className="mb-6 flex items-end justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-white">
                {savingsGoal
                  ? formatCurrency(savingsGoal, preferences.currency)
                  : "No goal set"}
              </h3>
            </div>
